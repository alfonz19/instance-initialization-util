...
public class CollectionConfiguration<ITEM_TYPE, GENERATES> implements Generator<GENERATES> {
    ...
    public CollectionConfiguration(Function<Collection<? extends ITEM_TYPE>, GENERATES> listInstanceSupplier, Generator<? extends ITEM_TYPE> itemGenerator) {
    ...
    @Override
    public GENERATES create() {
        List<? extends ITEM_TYPE> items = itemGenerator.create(sizeSpecification.getRandomSizeAccordingToSpecification());
        if (shuffled) {
